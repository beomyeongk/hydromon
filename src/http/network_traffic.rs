...
pub fn handle(request: Request, conn: &Connection) {
    ...
    let mut sql = "SELECT t.timestamp, n.name, t.rx_kbps, t.tx_kbps, t.rx_pckps, t.tx_pckps \
                   FROM network_traffic t JOIN name_map n ON t.name_id = n.id WHERE 1=1"
        .to_string();
    let mut sql_params: Vec<rusqlite::types::Value> = Vec::new();

    apply_time_filter(&params, &mut sql, &mut sql_params);

    sql.push_str(" ORDER BY t.timestamp ASC, n.name ASC");
    ...
    let body = NetworkTrafficResponse {
        size: data.len(),
        data,
    };
